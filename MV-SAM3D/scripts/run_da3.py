...
def run_da3_inference(
    ...
    if model_path is None:
        ...
        hf_cache_dir = Path.home() / ".cache" / "huggingface" / "hub" / "models--depth-anything--DA3NESTED-GIANT-LARGE"
        hf_snapshot = None
        if (hf_cache_dir / "snapshots").exists():
            snapshots = sorted((hf_cache_dir / "snapshots").iterdir())
            if snapshots:
                hf_snapshot = snapshots[-1]
        possible_paths = [
