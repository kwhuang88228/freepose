...
for result_filename in p["result_filenames"]:
    ...
    # Format: {method}_{dataset}-{split}[_{anything}].csv
    # Dataset names may contain underscores (e.g. hope_video), so we find the
    # first token that contains a '-' to locate the dataset-split boundary.
    result_name = os.path.splitext(os.path.basename(result_filename))[0]
    result_info = result_name.split("_")
    method = str(result_info[0])
    # Find the index of the first token containing '-' (dataset-split boundary)
    dash_idx = next(
        (i for i in range(1, len(result_info)) if "-" in result_info[i]), None
    )
    if dash_idx is None:
        raise ValueError(
            "Cannot parse dataset/split from filename: {}".format(result_filename)
        )
    dataset_info = result_info[dash_idx].split("-")
    # Tokens between method and dash_idx are part of the dataset name
    dataset = "_".join(result_info[1:dash_idx] + [dataset_info[0]])
    ...
    def inf_obj_path_tmpl(obj_id):
        glb_path = os.path.join(p["models_inference_path"], obj_id, obj_id + '.glb.obj')
        obj_path = os.path.join(p["models_inference_path"], obj_id, obj_id + '.obj')
        return glb_path if os.path.exists(glb_path) else obj_path
    ...
    for scene_ind, (scene_id, scene_targets) in enumerate(targets_org.items()):
        ...
        unique_inf_scene_obj_path = {inf_id: inf_obj_path_tmpl(inf_id)
