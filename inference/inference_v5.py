...
Pipeline:
    ...
    8.5 Apply relative-pose orientation: keep stage-8 translation, override
        orientation with a relative-rotation chain from HaMeR rel_poses.json
        anchored at frame 0
...
def main():
    ...
    # Stage 8.5: Apply relative-pose orientation
    # Keep the stage-8 per-frame translation (position estimation is already reliable)
    # but override the orientation with a relative-rotation chain from HaMeR's
    # rel_poses.json, anchored at frame 0's estimated orientation. Only the rotation
    # is propagated; the translation is untouched. Downstream symmetry canonicalization
    # and Kalman smoothing are intentionally not run.
    rel_poses_file = Path("/share/hariharan/kh775/code/hamer/inference/output") / video / "rel_poses.json"
    relpose_csv  = poses_csv.replace(".csv", "_relpose.csv")
    relpose_path = results_dir / relpose_csv
    if skip and relpose_path.exists():
        print(f"Stage 8.5: relpose output already exists ({relpose_csv}), skipping.")
    elif not rel_poses_file.exists():
        sys.exit(f"Stage 8.5: rel_poses.json not found at {rel_poses_file}")
    else:
