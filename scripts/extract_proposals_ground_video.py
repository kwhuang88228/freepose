...
def retrieve_meshes(tracking_output, retrieval, retrieval_features, filelist_path, layer=22, topk=0, device='cuda', ffa_dir=None):
    ...
    with torch.inference_mode():
        for frame_idx, output in tracking_output.items():
            ...
            proposals = Proposals(image, output, 420, 0, frame_idx, bbox_extend=0.1, mask_rgb=True)
            ...
            if feature_type == "cls":
                ...
                if ffa_dir is not None:
                    for prop_idx, prop_tensor in enumerate(proposals.proposals):
                        img_np = (prop_tensor.float().cpu().numpy().transpose(1, 2, 0) * 255).clip(0, 255).astype(np.uint8)
                        img_bgr = cv2.cvtColor(img_np, cv2.COLOR_RGB2BGR)
                        cv2.imwrite(str(ffa_dir / f"{frame_idx:06d}_ffa_image_{prop_idx:02d}.jpg"), img_bgr)
...
if __name__ == "__main__":
    ...
    # Save SAM2 bounding boxes and masks as separate overlaid images
    sam2_boxes_dir = results / "sam2_boxes"
    sam2_masks_dir = results / "sam2_masks"
    sam2_boxes_dir.mkdir(exist_ok=True)
    sam2_masks_dir.mkdir(exist_ok=True)
    ...
    logger.info(f"Saved SAM2 box overlays  → {sam2_boxes_dir}")
    logger.info(f"Saved SAM2 mask overlays → {sam2_masks_dir}")

    # Save raw binary masks (one PNG per object per frame) to masks/
    masks_dir = results / "masks"
    masks_dir.mkdir(exist_ok=True)
    for frame_idx, output in tracking_output.items():
        for obj_idx, mask in enumerate(output["masks"]):
            mask_np = (mask.cpu().numpy().astype(np.uint8)) * 255
            cv2.imwrite(str(masks_dir / f"{frame_idx:06d}_mask.png"), mask_np)
    logger.info(f"Saved raw binary masks   → {masks_dir}")
    ...
    all_proposals = retrieve_meshes(
        ...
        topk=args.topk,
        device=device,
        ffa_dir=ffa_dir if feature_type == "ffa" else None,
    )
    if feature_type == "ffa":
        logger.info(f"Saved FFA input crops → {ffa_dir}")

    # Render multiple views of each retrieved mesh
    import os as _os
    _os.environ.setdefault('PYOPENGL_PLATFORM', 'egl')
    import trimesh as _trimesh
    from src.pipeline.retrieval.renderer import MeshRenderer

    _mesh_views_dir = results / "mesh_views"
    _mesh_views_dir.mkdir(exist_ok=True)

    _first_proposals = all_proposals[min(all_proposals.keys())]
    _unique_meshes = list(dict.fromkeys(_first_proposals.meshes))

    _renderer = MeshRenderer(n_poses=600)
    _N_VIEWS = 6
    _view_indices = [int(i * 600 / _N_VIEWS) for i in range(_N_VIEWS)]

    _metadata_path = Path(__file__).parent.parent / "inference" / "metadata.json"
    with open(_metadata_path) as _f:
        _mesh_metadata = json.load(_f)

    for _mesh_id in _unique_meshes:
        _mesh_path = Path("data/mesh_cache") / _mesh_id / f"{_mesh_id}.glb.obj"
        if not _mesh_path.exists():
            logger.warning(f"Mesh not found: {_mesh_path}")
            continue
        _mesh = _trimesh.load_mesh(str(_mesh_path))
        if isinstance(_mesh, _trimesh.Scene):
            _mesh = _mesh.dump(concatenate=True)
        _mesh.apply_scale(0.25)
        _render_results = _renderer.render(_mesh)
        _view_imgs = [_render_results[vi][0] for vi in _view_indices]
        _strip = np.concatenate(_view_imgs, axis=1)
        _out_path = _mesh_views_dir / f"{_mesh_id}.jpg"
        cv2.imwrite(str(_out_path), cv2.cvtColor(_strip, cv2.COLOR_RGB2BGR))
        _object_name = _mesh_metadata.get(_mesh_id, "unknown")
        (_mesh_views_dir / "mesh_name.txt").write_text(_object_name)
        logger.info(f"Saved mesh views → {_out_path} ({_object_name})")
