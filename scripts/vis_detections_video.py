...
def vis_detections(img, bboxes, output_path, xywh=True, labels=None, scores=None):
    if not xywh:
    ...
    for idx, bbox in enumerate(bboxes):
        ...
        ax.add_patch(rect)
        label = labels[idx] if labels is not None and idx < len(labels) else ""
        score = scores[idx] if scores is not None and idx < len(scores) else None
        text = f"{idx} - {label}" if label else f"{idx}"
        text += f" ({score:.2f})" if score is not None else ""
        ax.text(x+10, y + 40, text, color="white", fontsize=12, bbox=dict(facecolor=color, alpha=0.5))
