...
class Sam3MultiplexBase(Sam3VideoBase):
    ...
    def _det_track_one_frame_impl(
        ...
        with torch.profiler.record_function("GPU sync and filter"):
            ...
            pos_pred_mask_idx = pos_pred_mask.to(torch.uint8).argsort(descending=True)
    ...
    def _propogate_tracker_one_frame_local_gpu(
        ...
        low_res_masks_list = []
        obj_scores_list = []
        for state_idx, inference_state in enumerate(inference_states):
            if len(inference_state["obj_ids"]) == 0:
                continue  # skip propagation on empty inference states

            # DEBUG: log state contents to diagnose the "No points are provided" crash
            try:
                n_obj = len(inference_state["obj_ids"])
                n_cond = len(inference_state["output_dict"]["cond_frame_outputs"])
                cond_frames = sorted(inference_state["output_dict"]["cond_frame_outputs"].keys())
                logger.info(
                    f"[DEBUG propagate] frame_idx={frame_idx} state_idx={state_idx} "
                    f"n_obj={n_obj} n_cond={n_cond} cond_frames={cond_frames} "
                    f"obj_ids={list(inference_state['obj_ids'])}"
                )
            except Exception as _e:
                logger.info(f"[DEBUG propagate] introspect failed: {_e}")
